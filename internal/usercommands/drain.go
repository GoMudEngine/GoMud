...
func Drain(rest string, user *users.UserRecord, room *rooms.Room, flags events.EventFlag) (bool, error) {
	if user.Character.IsActing() {
		user.SendText(messaging.CategorySystem, `<ansi fg="red">You can't drain while focused on your work. Finish or be interrupted first.</ansi>`)
		return true, nil
	}

	// Must be in combat or specify a target to use drain.
	if !user.Character.IsInCombat() {
		if rest == "" {
			user.SendText(messaging.CategorySystem, "Drain whom?")
			return true, nil
		}
		...
		if err != nil {
			...
			if pId, _ := room.FindByName(rest); pId == user.UserId {
				user.SendText(messaging.CategorySystem, "You can't drain yourself.")
				return true, nil
			}
