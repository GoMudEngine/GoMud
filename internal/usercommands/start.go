...
func Start(rest string, user *users.UserRecord, room *rooms.Room, flags events.EventFlag) (bool, error) {
	...
	if isNew {
		user.SendTextLegacy(``)
		user.SendTextLegacy(fmt.Sprintf(`You'll need to answer some questions.%s`, term.CRLFStr))
	}
	...
	if user.Character.SpeciesId == 0 {
		...
		if humanSpecies, ok := species.FindSpecies("human"); ok {
			...
			user.SendTextLegacy(``)
			user.SendTextLegacy(fmt.Sprintf(`  <ansi fg="magenta">*** Your form takes shape on the world of Gaius ***</ansi>%s`, term.CRLFStr))
		}
	}
	...
	if strings.EqualFold(user.Character.Name, user.Username) || user.Character.Name == user.TempName() || len(user.Character.Name) == 0 || strings.ToLower(user.Character.Name) == `nameless` {
		...
		if strings.EqualFold(question.Response, user.Username) {
			user.SendTextLegacy(`Your username cannot match your character name!`)
		...
		for _, c := range characters.LoadAlts(user.UserId) {
			if strings.EqualFold(question.Response, c.Name) {
				user.SendTextLegacy(`Your already have a character named that!`)
				question.RejectResponse()
				return true, nil
			}
		}
		...
		user.SendTextLegacy(fmt.Sprintf(`You will be known as <ansi fg="yellow-bold">%s</ansi>!%s`, user.Character.Name, term.CRLFStr))
	}
	...
	if duration.Hours() > 1 {
		...
		if question.Response != `no` {
			...
			user.SendTextLegacy(fmt.Sprintf(`<ansi fg="magenta">Suddenly, a vortex appears before you, drawing you in before you have any chance to react!</ansi>%s`, term.CRLFStr))
	...
	if err != nil {
		user.SendTextLegacy(`The Tutorial zone is fully occupied right now. Please try again in a few minutes`)
		return true, nil
	}
