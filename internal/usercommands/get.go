...
func Get(rest string, user *users.UserRecord, room *rooms.Room, flags events.EventFlag) (bool, error) {
	...
	if room.GetVisibility() < 1 && !user.Character.HasFlagFromAnySource(buffs.NightVision) {
		user.SendTextLegacy("You can't see anything to pick up!")
		return true, nil
	}
	...
	if len(args) > 0 && room.MatchesSealedCrate(strings.ToLower(args[len(args)-1])) {
		user.SendTextLegacy(`The shipping crate is sealed and bound for the caravan; you can't get into it.`)
		return true, nil
	}
		...
		if len(args) >= 2 {
			...
			if allLastArg == "bag" || allLastArg == "case" || allLastArg == "pouch" || allLastArg == "components" {
				if len(user.Character.ComponentItems) == 0 {
					user.SendTextLegacy(`Your component bag is empty.`)
				} else {
					...
					user.SendTextLegacy(fmt.Sprintf(`You move %d item(s) from your component bag to your backpack.`, ct))
				}
				return true, nil
			}
			if allLastArg == "bandolier" || allLastArg == "potions" {
				if len(user.Character.PotionItems) == 0 {
					user.SendTextLegacy(`Your bandolier is empty.`)
				} else {
					...
					user.SendTextLegacy(fmt.Sprintf(`You move %d potion(s) from your bandolier to your backpack.`, ct))
				}
				return true, nil
			}
		}
		...
		if len(args) >= 2 {
			...
			if cName != `` {
				...
				if container.Gold < 1 && len(container.Items) < 1 {
					user.SendTextLegacy(fmt.Sprintf(`There's nothing in the <ansi fg="container">%s</ansi>.`, cName))
				}
				return true, nil
			}
		}
	...
	// Handle "get all.item" — pick up all matching items from the floor
	{
		...
		if matchNum == -1 {
			...
			if picked == 0 {
				user.SendTextLegacy(fmt.Sprintf(`You don't see any "%s" to pick up.`, itemName))
			} else {
				user.SendTextLegacy(fmt.Sprintf(`You pick up %d item(s).`, picked))
	...
	if len(args) >= 2 {
		...
		if isBagGet || isBandolierGet {
			...
			if foundItem.ItemId == 0 {
				user.SendTextLegacy(fmt.Sprintf(`You don't see a "%s" in your %s.`, rest, label))
				return true, nil
			}
			...
			user.SendTextLegacy(fmt.Sprintf(`You move the <ansi fg="itemname">%s</ansi> from your %s to your backpack.`, foundItem.DisplayName(), label))
			return true, nil
		}
	...
	if petUserId == user.UserId {
		...
		if !found {
			user.SendTextLegacy(fmt.Sprintf(`You don't see a %s carried by %s.`, rest, user.Character.Pet.DisplayName()))
		} else {
	...
	if containerName != `` {
		...
		if args[0] == goldName || (len(args[0]) < 5 && goldName[0:len(args[0])-1] == args[0]) {
			...
			if container.Gold < 1 {
				user.SendTextLegacy("There's no gold to grab.")
			} else {
		...
		if !found {
			user.SendTextLegacy(fmt.Sprintf(`You don't see a %s in the <ansi fg="container">%s</ansi>.`, rest, containerName))
		} else {
		...
		// First try the requested source (floor or stash)
		{
			...
			if peekFound && peekItem.HasAdjective(`exploding`) {
				user.SendTextLegacy(`You can't pick that up, it's about to explode!`)
				return true, nil
			}
			if peekFound {
		...
		if len(foundNoun) > 0 {
			...
			user.SendTextLegacy(fmt.Sprintf(`You can't get the <ansi fg="noun">%s</ansi>`, foundNoun))
			room.SendTextVisualLegacy(fmt.Sprintf(`<ansi fg="username">%s</ansi> is grasping at the air.`, user.Character.Name), user.UserId)
	...
	if _, corpseFound := room.FindCorpse(rest); corpseFound {
		user.SendTextLegacy(`You can't pick up corpses. What would people think?`)
		return true, nil
	}
	...
	if containerName != `` {
		user.SendTextLegacy(fmt.Sprintf(`You can't pick up the <ansi fg="container">%s</ansi>. Try looking at it.`, containerName))
	} else {
		user.SendTextLegacy(fmt.Sprintf("You don't see a %s around.", rest))
	}
...
func sendEncumbranceWarning(user *users.UserRecord) {
	...
	if encPct >= 2.0 {
		user.SendTextLegacy(`<ansi fg="red-bold">You are severely overloaded!</ansi> Your combat and movement are heavily penalized.`)
	} else if encPct >= 1.5 {
		user.SendTextLegacy(`<ansi fg="red">You are heavily encumbered.</ansi> Your combat and movement are significantly penalized.`)
	} else if encPct >= 1.0 {
		user.SendTextLegacy(`<ansi fg="yellow">You are encumbered.</ansi> Your combat and movement are penalized.`)
	} else if encPct >= 0.75 {
		user.SendTextLegacy(`<ansi fg="yellow">You are carrying a moderate load.</ansi>`)
	}
