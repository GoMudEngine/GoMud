...
func AiFlag(rest string, user *users.UserRecord, room *rooms.Room, flags events.EventFlag) (bool, error) {
	...
	if rest == "" {
		user.SendText(`Usage: <ansi fg="command">ai-flag &lt;username&gt;</ansi> - Toggle AI flag on a user account.`)
		return true, nil
	}
	...
	if targetUser.IsAI {
		user.SendText(fmt.Sprintf(`<ansi fg="username">%s</ansi> (<ansi fg="username">%s</ansi>) has been flagged as <ansi fg="cyan-bold">AI</ansi>.`, targetUser.Username, targetUser.Character.Name))
	} else {
		user.SendText(fmt.Sprintf(`<ansi fg="username">%s</ansi> (<ansi fg="username">%s</ansi>) AI flag has been <ansi fg="alert-1">removed</ansi>.`, targetUser.Username, targetUser.Character.Name))
	}
...
func AiList(rest string, user *users.UserRecord, room *rooms.Room, flags events.EventFlag) (bool, error) {
	...
	for _, uid := range users.GetOnlineUserIds() {
		...
		if u.IsAI || isAIConn {
			...
			user.SendText(fmt.Sprintf(`  <ansi fg="username">%s</ansi> (<ansi fg="username">%s</ansi>)%s%s`, u.Username, u.Character.Name, flagTag, connTag))
		}
	}

	if aiCount == 0 {
		user.SendText("No AI accounts currently online.")
	} else {
		user.SendText(fmt.Sprintf("%d AI account(s) online.", aiCount))
	}
