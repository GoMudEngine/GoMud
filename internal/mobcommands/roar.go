...
// Roar is a bear conviction attack — a taunt reskin for mob use.
func Roar(rest string, mob *mobs.Mob, room *rooms.Room) (bool, error) {
	...
	if attackSuccess {
		...
		combat.RecordSpecialMove(combat.Mob, tgtType, "roar", true, dmg,
			&mob.Character, defender, util.GetRoundCount())
		...
		combat.RecordSpecialMove(combat.Mob, tgtType, "roar", false, 0,
			&mob.Character, defender, util.GetRoundCount())
	}
