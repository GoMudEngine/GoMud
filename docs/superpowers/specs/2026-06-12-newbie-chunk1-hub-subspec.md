...
marked `house` are indoor (weather-sheltered). Coordinates use the engine mapper convention documented in the
coord-budget doc (north = y-1, south = y+1, east = x+1, up = z+1); the
builder cross-checks one delta before mass-authoring.

| Id | Name | Biome | Coord | Exits (→ id) | Notes |
|---|---|---|---|---|---|
| 5200 | The Awakening Pool | water | (45,0,0) | W→5201, E→5202, S→5203, N→5211 | Zone root. The plunge pool. Rite site. Cleric Hadwen stationed here. |
...
| 5214 | Cleric's Study | house | (44,-3,0) | E→5213 | Hadwen's books; quiet lore. |
...
|---|---|---|---|---|
| 9100 | Cleric Hadwen | Awakening Rite + orientation; 5200 | Pale chitin plates along the jaw | d-400 |
...
| 9106 | Crier Toke | Hub square; 5203 | A second mouth at the throat (the loud one) | d-406 |
...
2. `ask hadwen rite` (triggers incl. `quest`/`task` per SOP) → dialogue
   grants `30-start` (`questExcluded: ["30-start", "30-end"]` per the
   re-grant SOP) and Hadwen describes the rite.
3. The rite beat: Hadwen's behavior tree responds to the player's
   follow-up (`ask hadwen begin` or speech keyword) — gated on quest token
...
4. Post-rite, Hadwen's `30-end`-gated root points the player at the
...
- Granted by Crier Toke (`ask toke quest`), requires `30-end`.
...
- Reward: a small purse of gold + Toke announces the seven coulee mouths
  (the spoke teaser). No items.

### Portal (not a quest — a permanent mechanism; NO JS by directive)

The Threshold House (5215) portal is driven by **Warden Esk's behavior
tree** using a NEW Go btree action — `teleport_player` — added in this
chunk (engine touch #2, see §6). Flow: the player asks Esk for passage
(`ask esk passage` / portal keywords, all hinted); Esk's btree, gated on
quest token `30-end`, fires `teleport_player` with destination
**Thornwall Temple Interior (room 468)** plus an in-character transit
sequence. Without the token, Esk explains the pool comes first.
Repeatable forever; this is the veteran exit.

Rationale (user directive 2026-06-12): the project deliberately removed
JS scripting wherever possible — debugging JS costs more than it returns.
`teleport_player` follows the exact precedent of `grant_mutation`
...
New action in `internal/behaviortree/actions_quest.go` (or a sibling
file), registered as `teleport_player`, params: `room_id` (int, required).
Teleports the TRIGGERING PLAYER (same player-resolution mechanics as
`grant_mutation`) via the engine's standard move path
(`rooms.MoveToRoom`-equivalent — builder uses whatever
`questengine/bridge.go:205 Teleport` uses so messaging/GMCP stay
consistent). Quest gating is handled by the btree's existing condition
nodes (same gating used elsewhere for quest-aware behavior — builder
...
| Lesson | Where |
|---|---|
| Awakening + `mutations` command | Hadwen, post-rite (Q30) |
| `help` / `help <topic>` literacy | Hadwen orientation line + school notice board noun |
| `ask <npc> <topic>` dialogue | every NPC; hints per SOP |
| `say`/`tell`/`emote` mention | Crier Toke flavor |
