# Quest Minimap-Marker Coverage Ledger

Generated 2026-07-21 (Phase 1). One row per quest; each cell is `step→room`, `step→N(inf)`
(room_enter inference), or `step→∅` (deliberate `-1`, no marker). `end` steps omitted.

Totals: **104 explicit targets · 7 inferred · 87 deliberate ∅** across 78 quests.
The ∅ steps carry an inline reason in the quest YAML; those tagged "Phase 2 split review"
are candidates for step-splitting (see `phase2_candidates` / the plan).

| Quest | Steps (step→marker) |
|-------|---------------------|
| 2 The Warren Compact | start→∅ |
| 4 The Warden's Report | start→∅ · investigate→∅ · evidence→405 |
| 5 The Innkeeper's Complaint | start→∅ |
| 6 The Collector's Burden | start→476 · report→421 |
| 7 The Fallow Field | start→∅ |
| 8 The City Watch's Missing Person | start→477 · found→473 |
| 9 The Temple's Tithe Audit | start→∅ |
| 10 The Drowning Post's Debt | start→473 · report→472 |
| 11 The Windwarden's Dilemma | start→∅ |
| 12 The Warden's Covenant | start→∅ · ritual→∅ |
| 13 The Prospector's Gambit | start→∅ · extraction→∅ |
| 14 The Undertow | start→∅ · explore→498(inf) · confront→∅ · evidence→∅ · report→∅ |
| 15 The Peddler's Overdue Freight | start→∅ |
| 16 The Herbalist's Shortage | start→∅ · forager→4027 |
| 17 The Caravan Guard | start→∅ |
| 18 Betta's Silence | start→∅ |
| 19 The Lake-Caves Bounty | start→∅ · shrimp→∅ · hunter→4110 · signed→∅ |
| 20 Ulla's Silence | start→4138 · workshop_carving→4123 · temple_pillars→4124 · garden_marker→4144 · chapel_altar→4145 · wardstone_altar→4130 · bone_shoals_cache→4106 · brindle_confession→4140 · kingfisher_found→∅ · vella_journal→4137 |
| 21 Newcomer's Path | start→5203 |
| 28 Waking to Gaius | start→∅ · hinted→∅ · looked→∅ · examined→6259(inf) · status_prompt→∅ · statused→6260(inf) · carry_prompt→∅ · got→∅ · invd→∅ · helped→6261(inf) · speaks_prompt→∅ · said→∅ · asked→6262(inf) · proving_prompt→∅ · warcried→∅ · saw_conditions→∅ · checked_cooldowns→∅ · attacked→∅ · cast_spike→∅ · tripped→∅ · fled→∅ |
| 29 Two Roads | start→5203 |
| 30 The Awakening | start→∅ |
| 31 Find Your Footing | start→5213(inf) · school→5207 · shop→∅ · equip→5205(inf) · lantern→5203 |
| 32 First Blood | start→5227 · strike→5227 · special→5227 · consider→5227 · verbosity→5227 |
| 33 Hold the Wash | start→∅ · scout→∅ · squatter→5234 |
| 34 Take the Tower | start→5242 |
| 35 First Heat | start→5245 · craft→5245 |
| 36 Pick the Vein | start→∅ · scavenger→∅ · lurker→∅ · ore→5251 |
| 37 Into the Vein | start→5260 |
| 38 First Brew | start→5265 · brew→5265 · drink→5265 |
| 39 Wade the Marsh | start→∅ · leech→∅ · throw→∅ · forage→5271 |
| 40 Heart of the Mire | start→5280 |
| 41 First Sign | start→5284 · track→5284 · forage→5284 |
| 42 The Hunt | start→∅ · hunt→∅ · rest→5290 · cook→∅ |
| 43 Break the Pack | start→5298 |
| 44 First Casting | start→5305 · cast→5303 |
| 45 The Unquiet Grove | start→∅ · kill→∅ · cast→5309 |
| 46 The Unfolded | start→5320 |
| 47 First Words | start→5324 · help→5324 |
| 48 The Standing Stones | start→5335 · taunt→5333 |
| 49 The Old Shrine | start→5343 · find→5341 |
| 50 First Shot | start→5351 · reload→5351 · shoot→5350 |
| 51 Across the Canyon | start→∅ · shoot→5356 · kill→5353 |
| 52 The Overlook | start→5364 |
| 53 Cull the Wash | start→∅ · firstkill→∅ |
| 54 Back to the Forge | start→5245 |
| 55 Another Brew | start→5265 · brew→∅ |
| 56 Running Game | start→∅ · hunt→5284 |
| 57 Tending the Grove | start→∅ |
| 58 Reading the Stones | start→5343 |
| 59 Range Practice | start→5354 · shoot→5356 |
| 60 The Long Road | start→5409 · waybill→5445 · manifest→5471 |
| 61 Hedgerow Toll | start→∅ · bandit→5395 · reach→5378 |
| 62 Adders in the Ford | start→5423 · adder→5425 |
| 63 Dock Rat | start→5526 · evidence→∅ · report→∅ |
| 65 The Street Sweeper's Secret | start→5624 · discover→5602 · report→∅ |
| 66 The Addict's Plight | start→5620 · escort→∅ · detox→∅ |
| 67 The Bloom Trail | start→5523 · front→5808 · lintel→6013 · delivery→6029 · source→6031 · witness→5525 · report→∅ |
| 68 The Cooperage Circle | start→5719 · evidence→∅ · choice→∅ |
| 69 The Gallery Cipher | start→5901 · rubbing→5916 · gallery→6007 · reading→5916 |
| 70 The Pre-Founding Web | start→∅ · witness→5706 |
| 71 The Tribute | start→5817 · vell→5820 · horst→5815 · ledger→5810 |
| 72 The Water Dispute | start→∅ |
| 73 The Margin Notation | start→6226 · chart→6131 · scrivener→6143 · map→6220 |
| 74 The Undercroft | start→∅ · ledger→6188 · record→6198 · survey→6189 · descent→6215 · reveal→∅ |
| 75 The Surveyor's Report | start→6303 · survey→6305 · intro→6315 · testimony→∅ |
| 76 The Disc | start→4023 · journal→∅ · attune→∅ |
| 77 The Truth |  |
| 78 The Convergence | start→∅ |
| 79 Commission Vitalis Bandolier | start→∅ |
| 80 Commission The Blackrazor | start→∅ |
| 81 Commission Wayfarer's Pack | start→∅ |
| 82 Commission Aegis of Mockery | start→∅ |
| 83 Commission Thornwall Harness | start→∅ |
| 84 Commission Seething Prism | start→∅ |
| 85 Commission Zephyr Treads | start→∅ |
| 86 Commission Hollow Choir Staff | start→∅ |
| 87 Commission Phial Of Second Birth | start→∅ |
